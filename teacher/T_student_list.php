...
for ($i =0 ;$i <=100 ;$i++) {
    $sql = "SELECT stu_id,Name,sit_po,login_time,logout_time,online  FROM member WHERE UserID = $i AND Status = 'USER' AND course_no = $course_no";
    ...
    $sql = "SELECT vote_no FROM compettitive WHERE stu_id = $id[$i] AND course_no = $course_no";
    if($id[$i] != "") {
        $result = mysqli_query($objCon, $sql);
        $row = mysqli_fetch_assoc($result);
        $score[$i] = $row["vote_no"];
