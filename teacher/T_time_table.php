...
$strSQL= "SELECT DISTINCT stu_id  FROM member WHERE Status = 'ADMIN'";
...
for ($i=0; $i < $rownum ; $i++) {
  $sql = "SELECT DISTINCT name  FROM member WHERE stu_id = '$stu[$i]'";
  $result = mysqli_query($objCon, $sql);
  $row = mysqli_fetch_assoc($result);
  $name[$i] = $row["name"];

  for ($j=0; $j < 7; $j++) {
    $sql = "SELECT DISTINCT time_str,time_end  FROM time WHERE stu_id = '$stu[$i]' AND  day ='$d[$j]'";
    $result = mysqli_query($objCon, $sql);
    $row = mysqli_fetch_assoc($result);
    $s[$i][$j] = $row["time_str"];
    $e[$i][$j] = $row["time_end"];
