...
include ("config.php");
$sql = "SELECT MAX(UserID) AS max FROM member WHERE Status = 'USER' AND course_no = '$course_no'";
...
for($i =0;$i <50;$i++){
    ...
    //echo $abc;
    $sql = "SELECT stu_id  FROM member WHERE Status = 'USER' AND UserID = $abc AND course_no = '$course_no' AND online = 1";
    $result = mysqli_query($objCon, $sql);
    $row = mysqli_fetch_assoc($result);
    $number = $row["stu_id"];
    //echo $number;
    if($number != NULL){
        $sql = "UPDATE surprise SET number = $number WHERE course_no = $course_no " ;
